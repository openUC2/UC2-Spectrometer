...
class MyVideoCapture:
	def __init__(self, calibration, video_source=0, 
		...
		self.width = 640
		self.height = 480
		self.intensity = [0] * (self.width-4) #array for intensity 
		#print(self.width)
	...
	def read(self):
		imgResp = request.urlopen(self.url)
	...
	def get_frame(self):
		try:
			img = self.read()
			print(img.shape)
			frame = cv2.resize(np.transpose(img,(1,0,2)), (320, 240)) #resize the live image
	...
	def get_graph(self):
		if True:
			...
			bwimage = np.transpose(bwimage,(1,0)) #resize the live image
